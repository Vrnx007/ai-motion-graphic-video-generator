...
interface SceneTimelineProps {
  ...
  /** Seek Remotion player to the first frame of a scene (global timeline). */
  onSeekToFrame?: (frame: number) => void;
  /** Split one scene into two rows (half duration each); may require regenerate for part 2. */
  onSplitScene?: (id: number) => void;
}
...
function startFrameForSceneId(scenes: Scene[], id: number): number {
  const fps = 30;
  let acc = 0;
  for (const s of scenes) {
    if (s.id === id) return acc;
    acc += Math.max(1, s.duration) * fps;
  }
  return 0;
}
...
export default function SceneTimeline({
  ...
  const handleSegmentClick = (sceneId: number) => {
    onSelectScene(sceneId);
    if (onSeekToFrame) {
      onSeekToFrame(startFrameForSceneId(scenes, sceneId));
    }
  };
    ...
    <div className="space-y-3">
          ...
          {scenes.map((scene, i) => {
            ...
            return (
              <button
                key={scene.id}
                type="button"
                className={`h-full cursor-pointer transition-all hover:brightness-125 border-0 p-0 ${isActive ? "ring-1 ring-white z-10" : ""}`}
                style={{
                  width: `${width}%`,
                  background: `hsl(${(i * 60) % 360}, 60%, 50%)`,
                }}
                onClick={() => handleSegmentClick(scene.id)}
                title={`${scene.title} (${scene.duration}s) — jump here`}
              />
            );
          })}
        </div>
      ...
      >
        {scenes.map((scene) => {
          ...
          return (
            <Reorder.Item key={scene.id} value={scene} className="flex-shrink-0">
              <motion.div
                layout
                onClick={() => handleSegmentClick(scene.id)}
                className={`relative w-48 p-3 rounded-xl border cursor-pointer transition-all group ${
                  isActive
              ...
              >
                ...
                <div className="flex items-center justify-between mb-2">
                  <span
                    className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md border text-[8px] font-black uppercase tracking-wider ${config.color}`}
                  >
                ...
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  {onSplitScene && scene.duration >= 6 && scenes.length < 24 && (
                    <button
                      type="button"
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="p-1 rounded-md bg-white/10 hover:bg-emerald-500/30 text-slate-400 hover:text-emerald-300 transition-colors"
                      title="Split into two scenes (half duration each)"
                    >
